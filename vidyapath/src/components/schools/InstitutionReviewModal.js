...
export default function InstitutionReviewModal({ institution, existingReview, onClose, onSubmitted }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (existingReview) {
        await api.updateInstitutionReview(institution._id, { ratings, title, comment, pros, cons });
      } else {
        await api.submitInstitutionReview(institution._id, { ratings, title, comment, pros, cons });
      }
      onSubmitted?.();
    } catch (e) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: '22px', fontWeight: '900', margin: 0 }}>{existingReview ? '✏️ Edit Review' : '⭐ Review Institution'}</h2>
          <p style={{ fontSize: '13px', opacity: 0.85, margin: '4px 0 0' }}>{institution?.name}</p>
        </div>
        ...
        <div style={{ padding: '24px' }}>
          ...
          {step === 1 && (
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: '800', marginBottom: '20px' }}>Rate each category</h3>
              ...
              <button onClick={() => { if (allRated) setStep(2); else setError('Please rate all categories'); }} style={{
                width: '100%', padding: '14px', marginTop: '24px', background: allRated ? '#0083B0' : '#E5E7EB',
          ...
          {step === 2 && (
            <div>
              ...
              <label style={{ display: 'block', marginBottom: '14px' }}>
                ...
                <textarea placeholder="Share your detailed experience..." value={comment} onChange={(e) => setComment(e.target.value)} style={{ ...inp, minHeight: '90px', resize: 'vertical' }} maxLength={1500} />
              </label>
              <label style={{ display: 'block', marginBottom: '16px' }}>
                <span style={{ fontSize: '13px', fontWeight: '600', color: '#4B5563', display: 'block', marginBottom: '6px' }}>Pros</span>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" placeholder="Add a pro" value={prosInput} onChange={(e) => setProsInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addPro(); } }} style={{ ...inp, flex: 1 }} />
              ...
              <label style={{ display: 'block', marginBottom: '16px' }}>
                <span style={{ fontSize: '13px', fontWeight: '600', color: '#4B5563', display: 'block', marginBottom: '6px' }}>Cons</span>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" placeholder="Add a con" value={consInput} onChange={(e) => setConsInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCon(); } }} style={{ ...inp, flex: 1 }} />
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button onClick={() => setStep(3)} style={{ flex: 1, padding: '14px', background: '#0083B0', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer' }}>Review →</button>
              </div>
            </div>
          )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                  {ratingCategories.map(cat => (
                    <div key={cat.key} style={{ textAlign: 'center', padding: '8px', background: '#fff', borderRadius: '10px' }}>
                      <div style={{ fontSize: '20px' }}>{'★'.repeat(ratings[cat.key] || 0)}{'☆'.repeat(5 - (ratings[cat.key] || 0))}</div>
