    ...
    const fetchUser = async () => {
      try {
        ...
        if (res.user && res.user.role === 'parent') {
          ...
          else if (['school','university'].includes(res.user.role)) router.push('/institution');
      ...
      <div className={styles.mainContent}>
        <header className={styles.topbar}>
          ...
          <div className={styles.searchWrap}>
            ...
            <input type="text" className={styles.searchInput} placeholder="Search schools, schemes..." />
          </div>
          <div className={styles.topbarRight}>
            <NotificationCenter />
            <div className={styles.userAvatar} onClick={() => router.push('/parent/profile')}>
              <div className={styles.avatarCircle}>
                {user?.profile?.avatar ? (
                  <img src={api.getImageUrl(user.profile.avatar)} alt="Parent" style={{width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover'}} />
                ) : '👨‍👩‍👧'}
              </div>
              <div>
                <div className={styles.userName}>{user?.profile?.firstName} {user?.profile?.lastName}</div>
                <div className={styles.userGrade}>Parent • {user?.parentProfile?.children?.length || 0} Children</div>
              </div>
            </div>
          </div>
        </header>
