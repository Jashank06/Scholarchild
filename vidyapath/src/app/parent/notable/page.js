...
export default function ParentNotablePage() {
    ...
    <div style={{ padding: '0 0 40px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ color: '#6B7280', margin: 0, fontSize: '14px' }}>
          Important announcements and notable information for parents
        </p>
      </div>
        ...
        <>
              ...
              <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))' }}>
                {featured.map(n => (
                  <div key={n._id} onClick={() => handleClick(n)}
                    style={{ background: 'white', borderRadius: '20px', border: '2px solid #FEF3C7', padding: '28px', cursor: n.link ? 'pointer' : 'default', boxShadow: '0 4px 20px rgba(251, 191, 36, 0.1)', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', position: 'relative', zIndex: 1 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#0B0B1A', margin: '0 0 6px' }}>{n.title}</h3>
                        {n.description && <p style={{ fontSize: '14px', color: '#6B7280', margin: '0 0 12px' }}>{n.description}</p>}
          ...
          <div>
            ...
            <div style={{ display: 'grid', gap: '14px', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))' }}>
              {regular.map(n => (
                <div key={n._id} onClick={() => handleClick(n)}
                  style={{ background: 'white', borderRadius: '16px', border: '1px solid #E5E7EB', padding: '20px', cursor: n.link ? 'pointer' : 'default' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: '700', padding: '3px 10px', borderRadius: '100px', background: '#F3F4F6', color: '#6B7280' }}>{n.category}</span>
                  </div>
                  <h3 style={{ fontSize: '16px', fontWeight: '800', color: '#0B0B1A', margin: '0 0 6px' }}>{n.title}</h3>
                  {n.description && <p style={{ fontSize: '13px', color: '#6B7280', margin: '0 0 12px' }}>{n.description}</p>}
