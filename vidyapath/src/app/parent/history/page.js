...
export default function ParentHistoryPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await api.getParentHistory();
  ...
  const sections = [
    ...
    },
    {
      title: 'Achievements',
      icon: '🏅',
      items: history.achievements.map((item) => `${item.childName}: ${item.title}`),
    },
    {
      title: 'Feedback',
      icon: '💬',
  ...
  return (
    <div>
      ...
      <p style={{ color: '#6B7280', marginBottom: '28px' }}>News, achievements, feedback, and reviews in one place.</p>
