...
const SERVICE_TYPES = [
  { value: 'application_help', label: "Child's Application Help", icon: '📝' },
  { value: 'document_verification', label: 'Document Support', icon: '📎' },
  { value: 'scholarship_guidance', label: 'Parental Guidance', icon: '🧭' },
  { value: 'technical_support', label: 'Technical Issue', icon: '⚙️' },
  { value: 'complaint', label: 'Feedback/Complaint', icon: '⚠️' },
  { value: 'other', label: 'General Query', icon: '❓' },
];
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
        <div>
          <h1 style={{ fontSize: '32px', fontWeight: '900', color: '#0B0B1A', marginBottom: '8px' }}>Support & Help 🛠️</h1>
          <p style={{ color: '#6B7280', fontSize: '16px' }}>Need assistance with your child's profile or have a query? We're here to help.</p>
        </div>
        <button 
      ...
      {showForm && (
        <div style={{
          ...
          <h3 style={{ fontSize: '22px', fontWeight: '800', marginBottom: '30px', color: '#0B0B1A' }}>Lodge a Support Ticket</h3>
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
              <div>
                <label style={{ fontSize: '11px', fontWeight: '800', color: '#9CA3AF', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Request Type</label>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <input name="subject" value={form.subject} onChange={handleChange} required placeholder="Quick summary"
                style={{ width: '100%', padding: '14px', border: '1px solid #E5E7EB', borderRadius: '16px', fontSize: '15px', outline: 'none' }} />
            </div>
            <div style={{ marginBottom: '30px' }}>
              <label style={{ fontSize: '11px', fontWeight: '800', color: '#9CA3AF', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Description</label>
              <textarea name="description" value={form.description} onChange={handleChange} required rows="5" placeholder="How can we assist you?"
            ...
            }}>{submitting ? 'Submitting...' : 'Send Request 🚀'}</button>
          </form>
        </div>
      )}

      {/* Requests History */}
      <h2 style={{ fontSize: '24px', fontWeight: '800', color: '#0B0B1A', marginBottom: '24px' }}>Your Requests</h2>
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px', color: '#6B7280' }}>Loading tickets...</div>
      ) : requests.length === 0 ? (
        <div style={{
          ...
          <h3 style={{ fontSize: '20px', fontWeight: '800', color: '#0B0B1A', marginBottom: '8px' }}>No support requests yet</h3>
          <p>Any assistance you ask for will be listed here.</p>
        </div>
      ) : (
