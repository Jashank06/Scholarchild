...
export default function ScholarshipsPage() {
  ...
  const fetchListings = async (cat, fltrs) => {
    try {
      setLoading(true);
      const params = { type: 'scholarship', limit: ITEMS_PER_PAGE };
      if (cat !== 'All') params.category = cat.toLowerCase();

      Object.entries(fltrs).forEach(([key, val]) => {
  ...
  return (
    <div className={styles.listingPage}>
      ...
      {loading ? (
        ...
        <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>No scholarships found matching your filters.</div>
      ) : (
