...
export default function SchemesPage() {
  ...
  const fetchListings = async (cat, fltrs) => {
    try {
      setLoading(true);
      const params = { type: 'scheme', limit: ITEMS_PER_PAGE };
    ...
    <div className={styles.listingPage}>
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderLeft}>
          <h1>🏛️ Govt. Schemes <span className={styles.countBadge}>{stats.total}</span></h1>
          <p>Central and state government welfare schemes for students</p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>No schemes found matching your filters.</div>
      ) : (
